...
class DynamicPatch {
    ...
    updateUI(file) {
        ...
        } else if (this.mode !== 'noswitch') {
            this.checkbox.checked = this.checkPatch(file, true) === "on";
        }
        if (this.target === 'number') {
            // dynamic + numberPatch only patches one offset
            const offset = this.patches[0].offset + this.patches[0].off.findIndex((s) => s === 'NN');
    ...
    applyPatch(file) {
        if (this.target === 'number') {
            ...
            // Copied from NumberPatch.applyPatch
            // Convert user inputted number to little endian
            const view = new DataView(new ArrayBuffer(this.size * 2));

            switch (this.size) {
                case 1:
                    view.setInt8(1, this.number.value, true);
                    break;
                case 2:
                    view.setInt16(1, this.number.value, true);
                    break;
                default:
            ...
            for (var i = 0; i < this.size; i++) {
                switch (this.size) {
                    case 1:
                        var val = view.getInt8(1);
                        break;
                    case 2:
                        var val = view.getInt16(1);
                        break;
                    default:
            ...
            if (Array.isArray(this.patches[i].offset)) {
                this.patches[i].offset.forEach((offset) => {
                        if (this.target === 'string') {
                            ...
                            this.patches[i].on = this.patches[i].on.map((patch, idx) => patch === 'XX' || patch === 'NN' ? file[offset + idx] : patch);
                            this.patches[i].off = this.patches[i].off.map((patch, idx) => patch === 'XX' || patch === 'NN' ? file[offset + idx] : patch);
                            replace(file, offset,
                                featureOn? this.patches[i].on : this.patches[i].off)
                        }
                    }
                ...
                if (this.target === 'string') {
                    ...
                    this.patches[i].on = this.patches[i].on.map((patch, idx) => patch === 'XX' || patch === 'NN' ? file[this.patches[i].offset + idx] : patch);
                    this.patches[i].off = this.patches[i].off.map((patch, idx) => patch === 'XX' || patch === 'NN' ? file[this.patches[i].offset + idx] : patch);
                    replace(file, this.patches[i].offset,
                        featureOn? this.patches[i].on : this.patches[i].off);
                }
            }
    ...
    checkPatchAll(file, updateUiFlag = false) {
        ...
        for(var i = 0; i < this.patches.length; i++) {
            ...
            if(offOffset.length > 0 && onOffset.length > 0) {
                console.error('illegal status detected');
            }
            if(offOffset.length > 0) {
                if (updateUiFlag) {
                    offOffset.forEach((offset) => {
                        console.log(offOffset);
    ...
    searchPatchOffset(file, search, offset) {
        ...
        Uint8Array.prototype.indexOfArr = function(searchElements, fromIndex) {
            ...
            for(var i = index, j = 0; j < searchElements.length && i < this.length; i++, j++) {
                if ((this.target !== 'string' && searchElements[j] === 'XX') || searchElements[j] === 'NN') {
                    continue;
                }
